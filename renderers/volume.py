...
def volume_rendering(field_fn, ray_origins, ray_directions, rng_key, t_near=0., t_far=1.):
    ...
    points, t_vals = stratified_random(ray_origins, ray_directions,
                                       ...
                                       n_samples=32,
                                       rng_key=rng_key)
    viewdirs = ray_directions / jnp.linalg.norm(ray_directions)

    colors, opacities = sample_radiance_field(field_fn, points, viewdirs)
    weights = accumulated_transmittance(opacities, t_vals)
    ...
    depth = jnp.einsum('ij,ij->i', weights, t_vals)
...
def volume_rendering_hierarchical(coarse_field, fine_field,
    ...
    colors, opacities = sample_radiance_field(coarse_field, points, viewdirs)
    weights = accumulated_transmittance(opacities, t_vals)

    # sample and render the fine model
    rng_key, _ = jax.random.split(rng_key)
    points, t_vals = importance_sampling(ray_origins, ray_directions, t_vals, weights,
                                         n_samples=128, rng_key=rng_key)
    colors, opacities = sample_radiance_field(fine_field, points, viewdirs)
    weights = accumulated_transmittance(opacities, t_vals)
...
# TODO: separate functions or just a single function would work?
